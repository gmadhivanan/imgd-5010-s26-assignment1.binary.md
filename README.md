...
The code is read from left to right. Each code starts at a radius of 0, then using 13 the code moves outwards a single level. The standard sequence for each level should be similar to:
*symbol color outline color offset pattern radialout*

## Symbols
For each level the user can enter a symbol. The symbols are words 1 -5
